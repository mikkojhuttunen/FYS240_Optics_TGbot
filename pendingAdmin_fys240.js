 ...
 * the practical way to get them off Railway when no persistent volume is attached (and a handy
 * way to fetch them even when one is).
 *
 ...
 *
 * Only Telegram IDs in ADMIN_USER_IDS may use it; everyone else is ignored silently (the command
 * is not listed in /help). It only works in a private chat with the bot, so the export is never
 * posted into the course group. Workflow after export: PENDING_QUESTIONS_fys240.md.
 */

const path = require('path');
const quiz = require('./quizGenerator_fys240');
const mvQuiz = require('./multivalueQuizGenerator_fys240');
...
function summaryText(s1, s2) {
  const lines = [
    ...
    `- Single-select: ${s1.total}${langList(s1.byLang)}${sectionList(s1.bySection)}`,
    `- Multi-select: ${s2.total}${langList(s2.byLang)}${sectionList(s2.bySection)}`,
  ];
 ...
 * @param {boolean} p.isPrivate            true if the command came from a private chat
 ...
 */
async function handlePendingCommand({ chatId, userId, isPrivate = true, arg, sendText, sendDocument }) {
  if (!limiter.isAdmin(userId)) return false;
  if (!isPrivate) {
    await sendText(chatId, 'Please use /pending in a private chat with the bot, so the export files are not posted into the group.');
    return true;
  }
  ...
  if (sent) await sendText(chatId, 'Save the file(s) into the repo folder, run: node mergePending_fys240.js review. Send /pending clear afterwards so they are not exported twice.');
  return true;
}
