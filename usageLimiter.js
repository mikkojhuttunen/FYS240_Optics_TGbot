 ...
 *   LLM_WEIGHT_CHAT           credit cost of a chat/Q&A answer or /HW hint (default 1)
 *   LLM_WEIGHT_QUIZ           credit cost of a quiz that needs live AI generation (default 1);
 *                             quizzes served purely from the question bank are free
 *   LLM_WEIGHT_CHECKHW        credit cost of a /checkhw submission      (default 3; the FYS.240 bot
 *                             has no /checkhw, so this is currently unused)
 ...
 *   CACHE_TTL                 same variable bot_fys240.js reads ("1h" default, or "5m"); only used
 *                             here to price prompt-cache writes (1h = 2x input, 5m = 1.25x)
 *
...
// bot_fys240.js defaults CACHE_TTL to "1h" (extended cache), whose cache WRITES cost 2x the base
// input price; the 5-minute cache costs 1.25x. The course corpus is ~80k tokens, so getting this
// multiplier wrong noticeably skews the spend estimate (and therefore the backstop).
function cacheWriteMultiplier() {
  return (process.env.CACHE_TTL || '1h') === '1h' ? 2 : 1.25;
}
...
function costUSD(model, usage) {
  ...
  return (
    (input * p.in + output * p.out + cacheRead * p.in * 0.1 + cacheWrite * p.in * cacheWriteMultiplier()) / 1e6
  );
}
