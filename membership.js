...
async function isCourseMember(bot, userId) {
  ...
  try {
    ...
    // bot_fys240.js talks to Telegram through axios, where err.message is just "Request failed with
    // status code 400" and Telegram's real reason is in err.response.data.description. Look at all the
    // places a description can live (axios: response.data, node-telegram-bot-api: response.body).
    const msg =
      [err?.response?.data?.description, err?.response?.body?.description, err?.message]
        .filter(Boolean)
        .join(' | ') || String(err);
